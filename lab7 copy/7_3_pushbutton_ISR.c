...
extern int period;  //본래 코드에 전역 변수로 지정했던 period 함수 참조
...
void pushbutton_ISR()
{
    ...
    *(KEY_Ptr+3)= press;	//edge capture register에 어떤 값을 다시 써서 초기화 함
    ...
    if(press == 0b10){  //눌러진 것이 KEY1이라면
        period = period / 2; //주기가 반 감소해야하므로 /2를 해야함
        *(TIMER_Ptr+1)=0b1011;		//control register의 STOP, CONT, ITO = 1
        config_timer(); //config_timer 호출
        
    }

    else if(press == 0b100){ //눌러진 것이 KEY2라면
         period = period * 2; //주기가 두 배 증가해야하므로 *2를 해야함
        *(TIMER_Ptr+1)=0b1011;		//control register의 STOP, CONT, ITO = 1
        config_timer(); //config_timer 호출
    }

	else if(press==0b1000)		//눌러진 것이 KEY3라면
	{
		...
		else if (run==0)			//run이 1이 아니라면 0이므로 stop 상태임
		{
